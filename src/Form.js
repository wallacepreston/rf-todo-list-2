...
const Form = props => {
  ...
  const {taskName, assignee, handleChange, handleSubmit, warningMessage, errorMessage } = props
...
return  <form id='todo-form' onSubmit={handleSubmit}>
    ...
    <input name='taskName' type='text' onChange={handleChange} value={taskName} />

    <label htmlFor='assignee'>
      Assign To:
      {!assignee && warningMessage && <span className='warning'>{warningMessage}</span>}
    </label>
    <input name='assignee' type='text' onChange={handleChange} value={assignee} />
